...
function getL10nCardMap(locale = DEFAULT_LOCALE) {
    ...
    let localeFilePath = new URL(`../../localizations/${locale}.json`, import.meta.url);
    if (!fs.existsSync(localeFilePath)) {
        localeFilePath = new URL(`../../localizations/${DEFAULT_LOCALE}.json`, import.meta.url);
    }
    const listMsgObj = JSON.parse(fs.readFileSync(localeFilePath, "utf8"))["translations"][""];
    const l10nCardMap = new Map();
    Object.keys(listMsgObj).forEach((msgName) => {
        l10nCardMap.set(listMsgObj[msgName]["msgstr"][0], listMsgObj[msgName]["msgid"]);
    });
    return l10nCardMap;
}
    ...
    return {
        ...
        blockquote(h, node) {
            ...
            if (type) {
                ...
                return h(node, "div", { className: isCallout ? [type] : ["notecard", type] }, wrap(all(h, node), true));
            }
        ...
        list(h, node) {
            ...
            const items = all(h, node).map((item) => ({
                ...item,
                children: item.children.flatMap((child) => child.tagName == "p" ? child.children : [child]),
            }));
