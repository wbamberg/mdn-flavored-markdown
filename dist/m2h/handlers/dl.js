...
export function asDefinitionList(h, node) {
    const children = node.children.flatMap((listItem) => {
        ...
        const definition = listItem.children[listItem.children.length - 1].children[0];
        const [paragraph, ...rest] = definition.children;
        paragraph.children[0].value = paragraph.children[0].value.slice(DEFINITION_PREFIX.length);
        return [
            h(node, "dt", {}, all(h, {
                ...node,
                children: terms.length == 1 && terms[0].type == "paragraph"
                    ? terms[0].children
                    : terms,
            })),
            h(node, "dd", {}, all(h, { ...definition, children: [paragraph, ...rest] })),
        ];
    });
